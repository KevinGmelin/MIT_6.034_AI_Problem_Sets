...
# discussed in class.
from production import IF, AND, OR, NOT, THEN, forward_chain, DELETE
...
same_identity_rule = IF(OR('male (?x)',
                           'female (?x)'),
...
brother_rule = IF(AND('parent (?x) (?y)',
                      'parent (?x) (?z)',
                      'male (?y)',
                      NOT('same-identity (?y) (?z)')),
                  THEN('brother (?y) (?z)'))

sister_rule = IF(AND('parent (?x) (?y)',
                     'parent (?x) (?z)',
                     'female (?y)',
                     NOT('same-identity (?y) (?z)')),
                 THEN('sister (?y) (?z)'))
...
# family_rules.
family_rules = [same_identity_rule, sibling_rule, brother_rule, sister_rule, mother_rule, father_rule, son_rule,
                daughter_rule, cousin_rule, grandparent_rule]
